...
import { useSecurityCardsData } from '@/lib/data';
import { useTranslation } from '@/hooks/useTranslation';

export default function SecurityClient() {
  const securityCards = useSecurityCardsData();
    ...
    <main className="flex-grow flex flex-col items-center justify-start p-8 pt-20 bg-[#fafbfe] dark:bg-gray-900 transition-colors">
      ...
      <div className="max-w-4xl mx-auto text-center flex flex-col items-center">
        ...
        <div className="inline-flex items-center space-x-2 bg-[#0a58d1] text-white px-3 py-1.5 rounded-full text-xs font-semibold mb-8 shadow-sm">
          <Lock size={14} />
          <span>{t('security.badge', 'Enterprise-Grade Privacy')}</span>
        </div>

        {/* Heading */}
        <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-6 tracking-tight">
          {t('security.heading', 'Your Security is Our Priority')}
        </h1>

        {/* Subtitle */}
        <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto leading-relaxed mb-10">
          {t('security.subtitle', 'Experience zero-knowledge document processing. We built dowll on the principle that your data belongs to you, processing it locally so it never leaves your device.')}
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4 mb-20">
          <Link href="/" className="bg-[#0f3482] hover:bg-[#0c2a68] text-white font-medium py-3 px-6 rounded-lg transition-colors text-sm shadow-sm">
            {t('security.button.start', 'Start Processing Securely')}
          </Link>
          <Link href="/legal/privacy" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-[#0f3482] dark:text-blue-400 hover:bg-gray-50 dark:hover:bg-gray-700 font-medium py-3 px-6 rounded-lg transition-colors text-sm shadow-sm">
            {t('security.button.privacy', 'Learn more about our Privacy Policy')}
          </Link>
        </div>
      </div>
