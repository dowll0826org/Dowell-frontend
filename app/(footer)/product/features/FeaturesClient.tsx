    ...
    <main className="flex-grow flex flex-col items-center justify-start p-8 pt-20 bg-[#fafbfe] dark:bg-gray-900 transition-colors">
      ...
      <div className="max-w-6xl mx-auto w-full mb-20 px-4">
        <div className="text-center mb-16">
          <p className="text-blue-600 dark:text-blue-400 font-bold tracking-widest uppercase text-[11px] mb-3">{t("features.sectionLabel")}</p>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">{t("features.sectionTitle")}</h2>
                ...
                <div key={card.id} className="md:col-span-2 bg-white dark:bg-gray-800 rounded-[20px] shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-blue-200 dark:hover:border-blue-900 group">
                  <div className="p-8 md:p-10 flex flex-col md:w-3/5">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3">
                      {card.id === 1 ? t('tools.merge-pdf', card.title) :
                       card.id === 2 ? t('tools.split-pdf', card.title) :
                       card.id === 3 ? t('tools.compress-pdf', card.title) :
                       card.id === 4 ? t('tools.convert-documents', card.title) :
                       card.id === 5 ? t('tools.edit-pdf', card.title) : card.title}
                    </h3>
                    <p className="text-gray-600 dark:text-gray-400 text-[15px] leading-relaxed flex-grow mb-8 pr-4">
                      {card.id === 1 ? t('guides.descriptions.desc-merge', card.description) :
                       card.id === 2 ? t('guides.descriptions.desc-split', card.description) :
                       card.id === 3 ? t('guides.descriptions.desc-compress', card.description) :
                       card.id === 4 ? t('guides.descriptions.desc-from-pdf', card.description) :
                       card.id === 5 ? t('guides.descriptions.desc-edit', card.description) : card.description}
                    </p>
